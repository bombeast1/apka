...
wss.on('connection', (ws) => {
  ...
  ws.on('message', (buf) => {
    ...
    if (type === 'registerAccount') {
      ...
      if (!uname || !password)
        return ws.send(
          JSON.stringify({ type: 'auth', ok: false, reason: 'BAD_INPUT' })
        );
    ...
    if (type === 'login') {
      ...
      ws.send(
        JSON.stringify({ type: 'auth', phase: 'login', ok: true, username: myName })
      );
      broadcastUsers();
      broadcastGroups();
      return;
    ...
    if (type === 'updatePublicKey') {
      ...
      if (rec) {
        rec.publicKeyJwk = publicKeyJwk || null;
      }
      broadcastUsers();
      return;
    }
    ...
    if (type === 'group-message') {
      ...
      for (const member of g) {
        ...
        if (target && target.ws && target.ws.readyState === 1) {
          target.ws.send(
            JSON.stringify({ type: 'group-message', from: myName, group, payload })
          );
        }
      }
      return;
